...
def relevant(task: Task, start_date: datetime, end_date: datetime) -> bool:
    ...
    if task.deadline is not None:
        result = result and task.deadline > start_date
        if task.start_date is not None:
            result = result and task.start_date < end_date
    return result
...
def count_deadline_metric(
    ...
    if task.deadline is None:
        return 0
    delta_deadline = (task.deadline - start_date).total_seconds()
    delta_start = (
        (end_date - task.start_date).total_seconds()
...
def count_time_tracking_metric(task: Task) -> float:
    ...
    left = max(task.plan - task.work, 0)
    return left / task.plan
